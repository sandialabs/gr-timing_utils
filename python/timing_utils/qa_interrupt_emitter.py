...
class qa_interrupt_emitter (gr_unittest.TestCase):
    ...
    def timemsg(self, abstime, fmt):
        if fmt == "sample":
            ...
            if fmt == "pair":
                ...
                return pmt.make_tuple(pmt.from_uint64(t_int), pmt.from_double(t_frac))
    ...
    def test_001_basic(self):
        ...
        t0, t1, t2 = .15, .25, .28
        self.tb.start()
        # wait until data has started flowing to issue the first interrupt instead of sleeping
        while self.tag_dbg.num_tags() == 0:
            time.sleep(1e-3)
        self.emitter.emit(self.timemsg(t0, "pair")) # issued at ~.100s due to start_time
        time.sleep(0.10)
        self.emitter.emit(self.timemsg(t1, "tuple")) # issued at ~.200s due to start_time
        time.sleep(0.06)
        self.emitter.emit(self.timemsg(t2 - self.start_time, "sample")) # issued at ~.260s due to start time
        for i in range(15):
