 ...
 * - freq (input):
 *        Input can be either:
 *           1) PMT pair (intern("freq"), double(frequency).
 *           2) PMT dict {intern("freq"), double(frequency)},
 *           3) PMT dict {intern("freq"), pmt::cons(double(frequency),double(phase))
 *        Continuous phase is maintained by not specifying an
 *        initial phase.  The frequency and phase are set asynchronously,
 *        with the new values being updated on the next subsequent
 *        work function call.
 *
 * The frequency and phase can also be set synchronously via stream tags.
 * Tags specified with the key \p {tag_key} are parsed for frequency and phase
 * updates.  The value of the tag can be either a single double value for the
 * frequency or as a pair with the first value being the frequency (double) and
 * the second is the phase (double).
 *
 * At the point the new frequency is applied to the signal, atag
 * is produced to let downstream blocks know when this has taken
 * affect.  Use the filter's group delay to determine when the
...
public:
     ...
     * \param phase Starting phase (radians)
     */
    virtual void set_center_freq(double center_freq, double phase = 0.0) = 0;
