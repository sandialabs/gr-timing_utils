...
class ReferenceTimer {
    public:        
        ReferenceTimer() : 
                error(0), run(false), loaded(false), index(0) {    
        ...
        ~ReferenceTimer() { StopTimer(); delete timer; }        
        ...
        void Timer(uint64_t interrupt_index) {
            boost::lock_guard<boost::mutex> guard(mtx);
            // It is possible for this function to be called while we are cancelling an interrupt.
            // Need to ensure that we don't process this.
            if (interrupt_index != index) {
                if (debug) printf("Skipping handler for old interrupt\n");
                return;
            }           
           
            if (debug) {
                double int_time = ((timer->expires_at() - epoch).total_microseconds())/1e6;            
                double diffval = (boost::get_system_time() - timer->expires_at()).total_microseconds();
            
                printf("interrupt_time = %f, clock_time = %f, diff = %f\n", int_time,
                    (boost::get_system_time() - epoch).total_microseconds()/1e6, diffval);
            }
        ...
        void StartTimer(double wait_time=0, pmt::pmt_t pmt_data=pmt::PMT_NIL) {
            ...
            if (loaded) {
                if (debug) printf("Received interrupt before previous one fired\n");
                timer->cancel();
            }
            loaded = true;
            d_out_pmt = pmt_data;
            index++;
            if (debug) printf("wait_time before interrupt = %f\n", wait_time);
            timer->expires_from_now(boost::posix_time::microseconds(1e6*wait_time));
            timer->async_wait(boost::bind(&ReferenceTimer::Timer, this, index));
        ...
        void UpdateTimer(double update_time) {
            ...
            if (loaded) {
                ...
                timer->cancel();
                index++;
                timer->expires_at(expire_time + boost::posix_time::microseconds(1e6*update_time));
                timer->async_wait(boost::bind(&ReferenceTimer::Timer, this, index));
            }
        }
